...
class L2Dataterm(Dataterm):
    def __init__(self, config):
        super(L2Dataterm, self).__init__(config)
...
class WeightedL1Dataterm(Dataterm):
    def __init__(self, config):
        super(WeightedL1Dataterm, self).__init__(config)
...
class L1Dataterm(Dataterm):
    def __init__(self, config):
        super(L1Dataterm, self).__init__(config)

    def energy(self, u, u0, gamma):
        return gamma*torch.sum(torch.abs(u - u0))

    def prox(self, u, u0, gamma, tau):
        return u0+torch.max(0, torch.abs(u-u0)-tau*gamma)*torch.sign(u-u0)

    def grad(self, u, u0, gamma):
        return gamma*torch.sign(u-u0) 